...
class ChessDatabaseGenerator:
    def __init__(self, take_database_file_open=False):
        self.take_database_file_open = take_database_file_open


        self.__database_file_content = None
        self.__check_file_content = None
    ...
    def store_games(self, database_file_name="database.json", check_file_name="check.json", store=True, verbose=0, backup=False):
        if not self.take_database_file_open:        # se è stato impostato di non tenere aperta la connessione con il file per non riaprirlo ogni volta: riapro i file
        ...
        else:       # se è stato impostato di non riaprire i file ogni volta
            if self.__database_file_content is None:
                self.__database_file_content = self.__read_database_file(database_file_name)

            database_file_content = self.__database_file_content


            if self.__check_file_content is None:
                self.__check_file_content = self.__read_check_file(check_file_name)

            check_file_content = self.__check_file_content
        ...
        # salvo le partite
        self.__store_games(database_file_content, check_file_content, database_file_name, check_file_name, store, verbose)
        


    # funzione effettiva
    def __store_games(self, database_file_content, check_file_content, database_file_name, check_file_name, store, verbose=0):
        #try:
            ...
            for game in games:
                ...
                self.__add_to_tree(self.__get_game_handle(game), database_file_content, verbose=verbose)
            ...
            # se è stato impostato di salvare salvo, altrimenti salvo come variabile di classe
            ...
            if store:
                ...
                self.__database_file_content = database_file_content
                self.__check_file_content = check_file_content
    ...
    '''
    def __add_to_tree(self, game, tree={}, lv=0, verbose=False):
        ...
        if len(game["moves"]) == lv:
            ...
            self.__add_to_tree(game, tree[game["moves"][lv]]["next"], lv + 1, verbose)
